...
class TechnoChiptuneEngine {
  ...
  nextNote() {
    ...
    // 90-second regeneration: only melody params change, NOT key/scale/progression
  ...
  startFade() {
    ...
    this.fadeEnd = this.fadeStart + 5.0;
  }
...
window.initAdvancedAudio = function() {
  if (window.bgmEngine) {
    if (window.bgmEngine.ctx && window.bgmEngine.ctx.state === 'suspended') {
      window.bgmEngine.ctx.resume();
    }
