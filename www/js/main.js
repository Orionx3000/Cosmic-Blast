...
let specialWeapon = null, cannonWeapon = null, weaponTimer = 0, isSnakeWave = false, isSwarmWave = false;
...
const DIFF = {
  easy:   { dropChance:0.7, hpMul:0.02, spdMul:0.75, fireMul:1.5, spawnMul:0.5, maxEnemies:200, health:8, dmgMul:0.8, rangeMul:1.0, aggressiveness:0.7, enemyFireRate:0.8, weaponTime:5, waveMul:1.4, mapScale:0.1, starMul:0.30, desc:'easier but enemies still fight back' },
  normal: { dropChance:0.65, hpMul:0.08, spdMul:0.85, fireMul:0.9, spawnMul:0.8, maxEnemies:300, health:6,  dmgMul:1.0, rangeMul:1.0, aggressiveness:1.0, enemyFireRate:1.0, weaponTime:5, waveMul:1.0, mapScale:0.25, starMul:0.70, desc:'balanced challenge' },
  hard:   { dropChance:0.65, hpMul:0.12, spdMul:0.85, fireMul:0.8, spawnMul:0.9, maxEnemies:400,health:5,  dmgMul:1.0, rangeMul:1.0, aggressiveness:1.0, enemyFireRate:1.1, weaponTime:4, waveMul:0.8, mapScale:0.5, starMul:1.20, desc:'tougher enemies' },
  hardcore: { dropChance:0.375, hpMul:0.35, spdMul:1.0, fireMul:0.7, spawnMul:1.2, maxEnemies:600, health:3, dmgMul:1.2, rangeMul:0.8, aggressiveness:1.2, enemyFireRate:1.3, weaponTime:4, waveMul:0.65, mapScale:1.0, starMul:2.00, desc:'HARDCORE: dense advancing swarms' }
};
...
const WEAPON_PITCH = {
  cannon:1.0,
  spread:1.0, shard:1.25, flare:0.80,
  rapid:1.0, ion:1.30,
  pierce:1.0,
  blast:1.0, gravity:0.65, nova:1.15, pulse:1.25, void:0.75, oblivion:0.55, singularity:0.45, pulsecannon:1.10, vortexcannon:0.90,
  beam:1.0, vortex:0.85, storm:1.15, tether:1.10, echo:0.90, vortexstorm:1.0,
  bolt:1.0,
  homing:1.0, ufo:0.80, phantom:1.25, phantomflare:1.15, glacier:1.10, nebula:0.70, rift:0.85,
  saber:1.0, prism:1.0, prismsaber:0.90, zenith:1.10, railgun:1.30, laser:1.0, time:0.80, siphon:0.85,
  swarm:1.15, starburst:1.0, inferno:0.70, mine:0.80
};
...
const STARS_LAYER = [
  ...
  { count:120, sz:0.5,  alpha:0.15, spd:0.18, fg:false },

  { count:80,  sz:0.7,  alpha:0.2,  spd:0.25, fg:false },

  { count:40,  sz:1.0,  alpha:0.28, spd:0.35, fg:false },

  { count:20,  sz:1.4,  alpha:0.36, spd:0.5,  fg:false },

  { count:10,  sz:1.8,  alpha:0.45, spd:0.7,  fg:false },

  { count:8,   sz:2.2,  alpha:0.55, spd:1.0,  fg:true  },

  { count:5,   sz:2.8,  alpha:0.7,  spd:1.4,  fg:true  }
...
function initAtmosphere() {
  ...
  // Soft glowing nebula gas (very faint and large, no polygons)
  ...
  for (let i = 0; i < 6; i++) {
    ...
    s.scale.set(10 + Math.random() * 15);

    s.alpha = 0.01 + Math.random() * 0.01;

    s.tint = Math.random() > 0.5 ? 0x661199 : 0x114488;
    ...
    mist.push({ 
      ...
      vy: (1 + Math.random() * 3) * (1 + layer * 0.3), 
  ...
  // Tiny stars as debris

  const texDebris = make(g=>{g.beginFill(0xffffff);g.drawCircle(0,0,1.5);});

  for (let i = 0; i < 120; i++) {

    const s = new PIXI.Sprite(texDebris);

    s.anchor.set(0.5);

    s.scale.set(0.5 + Math.random() * 1.5);

    s.alpha = 0.1 + Math.random() * 0.5;

    s.tint = Math.random() > 0.5 ? 0xaaaaff : 0xaaaaaa;
    ...
    debris.push({ sprite: s, vy: 5 + Math.random() * 30, vx: (Math.random() - 0.5) * 0.5, rv: (Math.random() - 0.5) * 0.1, phase: Math.random() * Math.PI * 2 });
...
function updateAtmosphere(dt, now) {
  ...
  if (bgTilingSprite) {
    ...
    bgTilingSprite.tilePosition.y += 10 * dt;
  ...
  for (const m of mist) {
    ...
    if (m.sprite.y > height + edge) { m.sprite.y = -edge; m.sprite.x = Math.random() * width; }
...
function playFireSnd(p) {
  p = p || 1;
  if (!audioCtx) return;
  const t = audioCtx.currentTime;
  const o = audioCtx.createOscillator(), g = audioCtx.createGain();
  o.type = 'sawtooth'; o.frequency.setValueAtTime((800 + Math.random() * 400) * p, t);
  o.frequency.exponentialRampToValueAtTime(200 * p, t + 0.04);
...
function playBeamSnd(p) {
  p = p || 1;
  if (!audioCtx) return;
  const t = audioCtx.currentTime;
  const o = audioCtx.createOscillator(), g = audioCtx.createGain();
  o.type = 'sine'; o.frequency.setValueAtTime(1800 * p, t); o.frequency.linearRampToValueAtTime(600 * p, t + 0.12);
  ...
  o2.type = 'sawtooth'; o2.frequency.setValueAtTime(300 * p, t); o2.frequency.linearRampToValueAtTime(120 * p, t + 0.1);
...
function playBoltSnd(p) {
  p = p || 1;
  if (!audioCtx) return;
  const t = audioCtx.currentTime;
  for (let i = 0; i < 3; i++) {
    const o = audioCtx.createOscillator(), g = audioCtx.createGain();
    o.type = i % 2 ? 'square' : 'sawtooth'; o.frequency.setValueAtTime((1200 + i * 600 + Math.random() * 300) * p, t + i * 0.02);
    o.frequency.exponentialRampToValueAtTime(200 * p, t + i * 0.02 + 0.04);
...
function playBlastSnd(p) {
  p = p || 1;
  if (!audioCtx) return;
  const t = audioCtx.currentTime;
  const o = audioCtx.createOscillator(), g = audioCtx.createGain();
  o.type = 'sawtooth'; o.frequency.setValueAtTime(200 * p, t); o.frequency.exponentialRampToValueAtTime(50 * p, t + 0.1);
  ...
  const o2 = audioCtx.createOscillator(), g2 = audioCtx.createGain();
  o2.type = 'sine'; o2.frequency.setValueAtTime(80 * p, t); o2.frequency.exponentialRampToValueAtTime(20 * p, t + 0.1);
...
function playPierceSnd(p) {
  p = p || 1;
  if (!audioCtx) return;
  const t = audioCtx.currentTime;
  const o = audioCtx.createOscillator(), g = audioCtx.createGain();
  o.type = 'sine'; o.frequency.setValueAtTime(2500 * p, t); o.frequency.linearRampToValueAtTime(3500 * p, t + 0.06);
  o.frequency.exponentialRampToValueAtTime(400 * p, t + 0.08);
...
function playHomingSnd(p) {
  p = p || 1;
  if (!audioCtx) return;
  const t = audioCtx.currentTime;
  const o = audioCtx.createOscillator(), g = audioCtx.createGain();
  o.type = 'sine'; o.frequency.setValueAtTime(600 * p, t); o.frequency.linearRampToValueAtTime(1400 * p, t + 0.06);
  o.frequency.exponentialRampToValueAtTime(800 * p, t + 0.08);
...
function playSpreadSnd(p) {
  p = p || 1;
  if (!audioCtx) return;
  const t = audioCtx.currentTime;
  for (let i = 0; i < 5; i++) {
    const o = audioCtx.createOscillator(), g = audioCtx.createGain();
    o.type = 'sawtooth'; o.frequency.setValueAtTime((1200 - i * 150 + Math.random() * 200) * p, t + i * 0.01);
    o.frequency.exponentialRampToValueAtTime(300 * p, t + i * 0.01 + 0.03);
    g.gain.setValueAtTime(0, t + i * 0.01); g.gain.linearRampToValueAtTime(0.006, t + i * 0.01 + 0.002);
    g.gain.exponentialRampToValueAtTime(0.001, t + i * 0.01 + 0.05);
    o.connect(g); g.connect((sfxGain || audioCtx.destination)); o.start(t + i * 0.01); o.stop(t + i * 0.01 + 0.04);
  }
}

function playRapidSnd(p) {
  p = p || 1;
  if (!audioCtx) return;
  const t = audioCtx.currentTime;
  const o = audioCtx.createOscillator(), g = audioCtx.createGain();
  o.type = 'square'; o.frequency.setValueAtTime((400 + Math.random() * 200) * p, t);
  o.frequency.exponentialRampToValueAtTime(100 * p, t + 0.02);
  g.gain.setValueAtTime(0, t); g.gain.linearRampToValueAtTime(0.004, t + 0.001);
  g.gain.exponentialRampToValueAtTime(0.001, t + 0.025);
  o.connect(g); g.connect((sfxGain || audioCtx.destination)); o.start(t); o.stop(t + 0.03);
}

function playSaberSnd(p) {
  p = p || 1;
  if (!audioCtx) return;
  const t = audioCtx.currentTime;
  const o = audioCtx.createOscillator(), g = audioCtx.createGain();
  o.type = 'sawtooth'; o.frequency.setValueAtTime(220 * p, t); o.frequency.linearRampToValueAtTime(440 * p, t + 0.04);
  o.frequency.linearRampToValueAtTime(330 * p, t + 0.1); o.frequency.linearRampToValueAtTime(550 * p, t + 0.15);
  ...
  const o2 = audioCtx.createOscillator(), g2 = audioCtx.createGain();
  o2.type = 'sine'; o2.frequency.setValueAtTime(110 * p, t); o2.frequency.linearRampToValueAtTime(165 * p, t + 0.12);
...
function playWeaponSnd(type) {
  ...
  if (!type) { playFireSnd(1 + (weaponLevels.cannon || 0) * 0.04); return; }

  const lvl = weaponLevels[type] || 0;
  const basePitch = WEAPON_PITCH[type] || 1;
  const pitchMul = basePitch * (1 + lvl * 0.04);

  switch (type) {

    case 'beam': playBeamSnd(pitchMul); break;

    case 'bolt': playBoltSnd(pitchMul); break;

    case 'blast': playBlastSnd(pitchMul); break;

    case 'pierce': playPierceSnd(pitchMul); break;

    case 'homing': playHomingSnd(pitchMul); break;

    case 'saber': playSaberSnd(pitchMul); break;

    case 'spread':
    case 'shard':
    case 'flare': playSpreadSnd(pitchMul); break;

    case 'rapid':
    case 'ion': playRapidSnd(pitchMul); break;

    case 'swarm':
    case 'starburst':
    case 'inferno':
    case 'mine': playFireSnd(pitchMul); break;
    ...
    case 'singularity':
    case 'pulsecannon':
    case 'vortexcannon': playBlastSnd(pitchMul); break;

    case 'vortex':
    case 'storm':
    case 'tether':
    case 'echo':
    case 'vortexstorm': playBeamSnd(pitchMul); break;
    ...
    case 'glacier':
    case 'nebula':
    case 'rift': playHomingSnd(pitchMul); break;
    ...
    case 'laser':
    case 'time':
    case 'siphon': playBeamSnd(pitchMul); break;

    default: playFireSnd(pitchMul);
...
function playHitSnd() {
  ...
  if (!audioCtx) return;
  if (window.bgmEngine && window.bgmEngine.scale) {
    const t = audioCtx.currentTime;
    const o = audioCtx.createOscillator();
    const g = audioCtx.createGain();
    const scale = window.bgmEngine.scale;
    const baseRoot = window.bgmEngine.baseRoot || 45;
    const noteIdx = scale[Math.floor(Math.random() * scale.length)];
    const octave = 24 + Math.floor(Math.random() * 3) * 12;
    const midi = baseRoot + octave + noteIdx;
    const freq = 440 * Math.pow(2, (midi - 69) / 12);
    o.type = Math.random() < 0.7 ? 'square' : 'sawtooth';
    o.frequency.setValueAtTime(freq, t);
    o.frequency.exponentialRampToValueAtTime(freq * 0.5, t + 0.04);
    o.frequency.exponentialRampToValueAtTime(freq * 0.1, t + 0.08);
    g.gain.setValueAtTime(0, t);
    g.gain.linearRampToValueAtTime(0.025, t + 0.002);
    g.gain.exponentialRampToValueAtTime(0.001, t + 0.08);
    o.connect(g); g.connect(sfxGain || audioCtx.destination); o.start(t); o.stop(t + 0.08);
  } else {
    const t = audioCtx.currentTime;
    const o = audioCtx.createOscillator(), g = audioCtx.createGain();
...
function playPopSnd(tier) {
  ...
  if (window.bgmEngine && window.bgmEngine.scale) {
    ...
    const scale = window.bgmEngine.scale;
    const baseRoot = window.bgmEngine.baseRoot || 45;
    const baseMidi = baseRoot + 24 + tier * 3;
    for (let i = 0; i < Math.min(3 + tier, 6); i++) {
      const o = audioCtx.createOscillator(), g = audioCtx.createGain();
      const noteIdx = scale[(i + Math.floor(Math.random() * 2)) % scale.length];
      const midi = baseMidi + noteIdx + Math.floor(i / 2) * 12;
      const freq = 440 * Math.pow(2, (midi - 69) / 12);
      o.type = 'triangle';
      o.frequency.setValueAtTime(freq, t + i * 0.03);
      g.gain.setValueAtTime(0, t + i * 0.03);
      g.gain.linearRampToValueAtTime(0.025 - i * 0.003, t + i * 0.03 + 0.002);
  ...
  if (specialWeapon === 'spread') {
    ...
    for (let bi = 0; bi < burstCount; bi++) {
      ...
      if (l) {
        ...
        l.ufoExplode = true; l.explodeRadius = 60 + lvl * 10; l.explodeDmg = 15 + lvl * 3;
      }
    }
...
function mkLaser(ang, tint, szMul, big, ox, oy) {
  ...
  const l = {
    ...
    life: Math.max(big ? 0.5 : 0.6, minLife * 0.7), pierce: !specialWeapon, blast: false,
...
function spawnBeam(lvl) {
  ...
  const beam = { ang, len, lvl: lvl || 0, life: 0.12, active: true };
...
function checkBeamCollision(b) {
  ...
  for (let i = enemies.length - 1; i >= 0; i--) {
    ...
    if (ddx * ddx + ddy * ddy < (e.size * 0.5 + 16) * (e.size * 0.5 + 16)) {
      ...
      e.hp -= Math.round((1 + Math.floor(Math.sqrt(wave))) * getDifficulty().dmgMul * adj);
      ...
      if (e.hp <= 0 && !e.dead) { e.dead = true;
        ...
        if (_t) {
          ...
          if (Math.random() < ((getDifficulty().dropChance || 0.4) * 0.25)) spawnSpecialPowerup(e.x, e.y);
...
function chainLightning(startEnemy, lvl, startX, startY, options = {}) {
  ...
  for (let ci = 0; ci < chained.length; ci++) {
    ...
    if (t.hp <= 0 && !t.dead) { t.dead = true;
      ...
      if (tier) { const pts = tier.pt; score += pts; playPopSnd(t.tier); spawnExplosion(t.x, t.y, tier.cl[0], 16 + lvl * 2); spawnPowerup(t.x, t.y); if (Math.random() < ((getDifficulty().dropChance || 0.4) * 0.25)) spawnSpecialPowerup(t.x, t.y); updateUI(); }
...
function spawnPowerup(x, y, forceWeaponType) {
  ...
  // Clamp spawn position to visible area so off-screen kills still drop visible loot
  if (gameC) {
    const z = gameC.scale.x || 1;
    const cx = gameC.x || 0;
    const cy = gameC.y || 0;
    const visLeft = -cx / z;
    const visRight = (width - cx) / z;
    const visTop = -cy / z;
    const visBottom = (height - cy) / z;
    x = Math.max(visLeft + 20, Math.min(visRight - 20, x));
    y = Math.max(visTop + 20, Math.min(visBottom - 100, y));
  }

  const d = getDifficulty();
  const shieldChance = 0.25 + d.dropChance * 0.1;
  ...
  if (isShield) {
    ...
    const standardWeapons = ['spread','rapid','pierce','blast','beam','bolt','homing','saber','gravity','nova','swarm','storm','shard','prism','flare','vortex','echo','tether','rift','pulse','mine','void','glacier','nebula','laser','ion','siphon','time','zenith','singularity','phantom','inferno','railgun','starburst','vortexstorm','prismsaber','phantomflare','oblivion','ufo'];
    const weights = {};
    standardWeapons.forEach((wpn, i) => {
      if (i < 10) weights[wpn] = 3;
      else if (i < 20) weights[wpn] = 2;
      else weights[wpn] = 1;
    });
    const weighted = [];
    Object.entries(weights).forEach(([wpn, wgt]) => {
      for (let i = 0; i < wgt; i++) weighted.push(wpn);
    });
  ...
  const pu = {
    ...
    sparkles, puSparkle, puChromaR, puChromaB, neonDot, neonDot2, neonC, spawnTime: performance.now()
...
function updateWeaponAmmoDisplay() {
  ...
  if (WEAPONS[specialWeapon].time) {
    ...
    const maxSecs = (weaponMaxTimerMs / 1000).toFixed(1);

    el.innerText = secs + 's / ' + maxSecs + 's';
...
function updateWeaponDial() {
  ...
  const validOwned = ownedWeapons.filter(w => weaponLevels[w] > 0);

  const base = ['cannon'];

  const all = base.concat(validOwned);
...
function takeDamage(isBulletHit) {

  if (invincible > 0) return;
  const shieldLoss = isBulletHit ? 0.1 : 0.5;
  if (shieldLayers > 1) {

    shieldLayers = Math.max(1, shieldLayers - shieldLoss);
...
function showVictory(waveCompleted = false) {
...
function showToast(msg, color = '#0ff', duration = 3000) {
  ...
  el.innerText = msg;
  tc.appendChild(el);
  setTimeout(() => { if (el.parentNode) { el.style.opacity = '0'; el.style.transition = 'opacity 0.5s'; setTimeout(() => el.remove(), 500); } }, duration);
}
...
function returnToMainMenu() {
  ...
  zoom = 0.5; zoomScale = 0.5;

  if (gameC) setZoom(0.5);

  document.getElementById('zoomLabel').innerText = '2x';
  ...
  // Hide game containers and destroy map container before showing main menu
  ...
  showDifficulty();  // Use showDifficulty to show continue option
...
mainMenuBtn.addEventListener('pointerdown', e => { e.preventDefault(); e.stopPropagation(); try { e.target.setPointerCapture(e.pointerId); returnToMainMenu(); } catch(e) { console.error(e); const errText = new PIXI.Text(e.stack, {fill:0xff0000, fontSize:12, wordWrap:true, wordWrapWidth:800}); errText.y = 100; app.stage.addChild(errText); location.reload(); } });
...
function resumeGame(s) {
  ...
  usedContinueThisNode = s.usedContinueThisNode || true;
  ...
  if (mapMode && s.mapNodes && s.mapNodes.length) {
    ...
    // Fix: if the saved current node was never completed (e.g. player quit mid-node),
    // revert to the last completed node or start node so the player can't warp past unplayed stars
    const curNode = mapNodes[mapCurrentIdx];
    if (curNode && !curNode.completed && !curNode.isGoal) {
      let foundCompleted = false;
      for (let i = mapCurrentIdx - 1; i >= 0; i--) {
        if (mapNodes[i] && mapNodes[i].completed) {
          mapCurrentIdx = i;
          foundCompleted = true;
          break;
        }
      }
      if (!foundCompleted) mapCurrentIdx = 0;
    }

    // Allow replaying the current node by clearing completed flag
    const cur = mapNodes[mapCurrentIdx];
    if (cur && cur.completed) { cur.completed = false; cur.visited = false; }
...
function showDifficulty() {
  ...
  // Destroy map container so it can't render behind the overlay
  if (mapContainer) { app.stage.removeChild(mapContainer); mapContainer.destroy({children:true}); mapContainer = null; mapContent = null; }
  mapActive = false;

  // Hide game containers so the canvas doesn't show stars/game behind the overlay
  if (starC) starC.visible = false;
  if (mistC) mistC.visible = false;
  if (fgStarC) fgStarC.visible = false;
  if (debrisC) debrisC.visible = false;
  ...
  setTimeout(() => { diffOverlay.style.pointerEvents = ''; }, 200);
...
function continueGame() {
  ...
  // Calculate cannon base Y accounting for zoom
  const z = gameC ? gameC.scale.x || 1 : 1;
  const cy = gameC ? gameC.y : 0;
  const visibleBottom = (height - cy) / z;
  const cannonBaseY = visibleBottom - 44 / z;

  cannonSprite.y = cannonBaseY;

  if (typeof cannonGlow !== 'undefined') { cannonGlow.x = CANNON_X; cannonGlow.y = cannonBaseY; }

  if (typeof cannonBase !== 'undefined') { cannonBase.x = CANNON_X; cannonBase.y = cannonBaseY; }

  shieldGraphic.x = CANNON_X; shieldGraphic.y = cannonBaseY; shieldGraphic.clear();

  peopleText.x = CANNON_X; peopleText.y = cannonBaseY + 24;

  if (typeof shipHull !== 'undefined') { shipHull.visible = true; shipHull.position.set(CANNON_X, cannonBaseY); rebuildShipHull(); }
...
const FACTIONS = [
  ...
  { name:'Crimson Fleet',   body:0xcf1717, accent:0x0eaf0e, ring:0x5252e0, boss:0x751515, core:0xd19494, enemy:0x0bd0d0 },

  { name:'Cobalt Covenant', body:0xcf7317, accent:0x0eaf5e, ring:0x9952e0, boss:0x754515, core:0xd1b394, enemy:0x0b6ed0 },

  { name:'Verdant Swarm',   body:0xcfcf17, accent:0x0eafaf, ring:0xe052e0, boss:0x757515, core:0xd1d194, enemy:0x0b0bd0 },

  { name:'Violet Imperium', body:0x7adc18, accent:0x0f66bd, ring:0xe35fa1, boss:0x4d8217, core:0xbad69f, enemy:0x750cdf },

  { name:'Solar Dominion',  body:0x12e212, accent:0x0a0ac2, ring:0xe75a5a, boss:0x138613, core:0x9dd79d, enemy:0xe506e5 },

  { name:'Crystal Collective',body:0x12e27a, accent:0x660ac2, ring:0xe7a15a, boss:0x13864d, core:0x9dd7ba, enemy:0xe50675 },

  { name:'Nebula Wardens',  body:0x18ecec, accent:0xd00bd0, ring:0xe9e967, boss:0x159393, core:0xa8dcdc, enemy:0xf40606 },

  { name:'Obsidian Order',  body:0x1882ec, accent:0xd00b6e, ring:0xa8e967, boss:0x155493, core:0xa8c2dc, enemy:0xf47d06 },

  { name:'Jade Dynasty',    body:0x1212f3, accent:0xd60505, ring:0x63ee63, boss:0x111197, core:0xa6a6dd, enemy:0xfafa00 },

  { name:'Amber Cartel',    body:0x8a20f3, accent:0xe57506, ring:0x71efb0, boss:0x5c12a5, core:0xc9b1e2, enemy:0x85ff0a },

  { name:'Sapphire Accord', body:0xf320f3, accent:0xe5e506, ring:0x71efef, boss:0xa512a5, core:0xe2b1e2, enemy:0x0aff0a },

  { name:'Ruby Conglomerate',body:0xf3208a, accent:0x75e506, ring:0x71b0ef, boss:0xa5125c, core:0xe2b1c9, enemy:0x0aff85 },
...
function generateMap() {
  ...
  const mapScale = (getDifficulty() && getDifficulty().mapScale) || 0.25;
  ...
  const starMul = d.starMul || 1.0;

  const targetStars = cols * rows * 0.42 * starMul; // ~1800 stars for 60x78 grid
  ...
  for (let cx = 0; cx < cols; cx++) {
    ...
    for (let cy = 0; cy < rows; cy++) {
      ...
      if (type !== 'empty') {
        ...
        const baseWaves = Math.random() < 0.35 ? 3 + Math.floor(Math.random() * 3) : Math.random() < 0.65 ? 4 + Math.floor(Math.random() * 4) : 6 + Math.floor(Math.random() * 4);

        const typeMul = type === 'boss' ? 1.5 : type === 'station' || type === 'depot' ? 0.6 : type === 'encounter' ? 0.8 : 1.0;

        const diffMul = (getDifficulty() && getDifficulty().waveMul) || 1;
  ...
  for (const node of mapNodes) {
    ...
    for (let need = Math.max(2 - reachable, minForward - forward); need > 0; need--) {
      ...
      for (let a = 0; a < 40; a++) {
        ...
        const wc = Math.max(4, Math.min(15, Math.round((4 + Math.random() * 12) * ((getDifficulty() && getDifficulty().waveMul) || 1))));
...
function renderMap() {
  ...
  mapZoom = Math.max(0.3, Math.min(width / mapW * 0.95, (height * 0.9) / mapH));

  mapScrollX = width / 2 - mapCenterX * mapZoom;

  mapScrollY = height / 2 - mapCenterY * mapZoom;

  mapContent.position.set(mapScrollX, mapScrollY);

  mapContent.scale.set(mapZoom);

  // Use actual node bounds for background sizing
  const bgPad = 100;
  const bgW = mapW + bgPad * 2;
  const bgH = mapH + bgPad * 2;
  const bgMinX = minX - bgPad;
  const bgMinY = minY - bgPad;
  ...
  const safeTop = 60; // Clear phone status bar/notch



  // === UI overlay - centered stack ===
  ...
  title.anchor.set(0.5); title.x = width / 2; title.y = safeTop;
  ...
  if (cur && cur.type !== 'goal') {
    ...
    ni.anchor.set(0.5); ni.position.set(width / 2, safeTop + 42);
  ...
  backText.x = 12; backText.y = safeTop + 12;

  backText.eventMode = 'static'; backText.cursor = 'pointer'; backText.hitArea = new PIXI.Rectangle(-12, -12, 120, 40);
  ...
  statsText.x = width - 6; statsText.y = safeTop; statsText.anchor.set(1, 0);
  ...
  legendBtn.x = width - 6; legendBtn.y = safeTop + 18; legendBtn.anchor.set(1, 0);

  legendBtn.eventMode = 'static'; legendBtn.cursor = 'pointer'; legendBtn.hitArea = new PIXI.Rectangle(-80, -12, 120, 40);
  ...
  function buildLegend() {
    ...
    legendPanel.x = width - 6 - pw; legendPanel.y = safeTop + 62;
  ...
  legendBtn.on('pointerdown', (e) => { e.stopPropagation(); legendExpanded = !legendExpanded; legendPanel.visible = legendExpanded; legendBtn.alpha = legendExpanded ? 1 : 0.5; });
...
function setupMapCanvasEvents() {
  ...
  function pu(e) {
    ...
    if (ptr && beforeDelete === 1) {
      ...
      if (Math.hypot(dx, dy) < 20 && Date.now() - ptr.t < 500) {
...
function handleMapTap(cx, cy) {
  ...
  for (const node of mapNodes) {
    ...
    if (d < bestD && d < 200) { bestD = d; best = node; }
...
function beginNodePlay(idx) {
  ...
  // Calculate cannon base Y accounting for zoom (same logic as game loop)
  const z = gameC ? gameC.scale.x || 1 : 1;
  const cy = gameC ? gameC.y : 0;
  const visibleBottom = (height - cy) / z;
  const cannonBaseY = visibleBottom - 44 / z;

  cannon.x = CANNON_X; cannon.y = cannonBaseY;
  ...
  cannonSprite.position.set(CANNON_X, cannonBaseY);

  cannonGlow.position.set(CANNON_X, cannonBaseY);

  cannonBase.position.set(CANNON_X, cannonBaseY + 10);

  shieldGraphic.position.set(CANNON_X, cannonBaseY);

  peopleText.position.set(CANNON_X, cannonBaseY + 24);

  if (shipHull) { shipHull.position.set(CANNON_X, cannonBaseY); gameC.addChild(shipHull); }
...
} catch(err) { console.error('beginNodePlay err', err.stack); gameState = 'playing'; pauseBtn.style.display = 'block'; } }
...
function finishNodePlay() {
  ...
  // All stars reachable from current are done?

  const cur = mapNodes[mapCurrentIdx];

  const hasMore = cur ? mapNodes.some(n => {

    if (n.completed || n.visited || n.idx === mapCurrentIdx) return false;

    return Math.hypot(n.x - cur.x, n.y - cur.y) <= TRAVEL_RADIUS;

  }) : false;

  showToast('NODE COMPLETE');

  setTimeout(() => showMapView(), 300);
...
let diffPicking = false;

bindBtn('startGameBtn2', e => {
  e.preventDefault();
  if (diffPicking) return;
  diffPicking = true;
  const diff = document.getElementById('diffSelect').value;
  setTimeout(() => { diffPicking = false; setDifficulty(diff); }, 150);
});
...
app.ticker.add(() => { try {
  ...
  if (isFiring && gameState === 'playing') {
    ...
    if (fireTimer >= fireRate) {
      ...
      fireTimer -= fireRate;
      if (fireTimer > fireRate * 2) fireTimer = fireRate * 2;
      ...
      if (specialWeapon && !WEAPONS[specialWeapon].time) {
        ...
        if (specialWeapon === 'spread') ammoCost = 1;

        else if (specialWeapon === 'rapid') ammoCost = 1;

        else if (specialWeapon === 'swarm') ammoCost = 1;

        else if (specialWeapon === 'prism') ammoCost = 1;

        else if (specialWeapon === 'vortex') ammoCost = 2;

        else if (specialWeapon === 'pulsecannon') ammoCost = 1;

        else if (specialWeapon === 'vortexcannon') ammoCost = 1;

        else if (specialWeapon === 'ufo') ammoCost = 2;
  ...
  if (waveTimer >= waveDuration) {
    ...
    if (mapMode && mapNodeWaveTotal > 0) {
      ...
      weaponLevels.cannon = 1 + Math.floor(wave / 5);

      waveTimer = 0;

      waveEl.innerText = 'WAVE ' + (wave) + ' / ' + mapNodeWaveTotal;

      if (specialWeapon) refillWeapon(specialWeapon);
  ...
  if (!boss) {
    ...
    const baseRate = Math.max(0.1, 0.7 - wave * 0.012) / (d.spawnMul * spawnAggression * mapSpawnBoost) * (zoom || 1) / superMul;
    ...
    if (spawnTimer >= baseRate) {
      ...
      if (Math.random() < (0.3 + wave * 0.02) * d.spawnMul * spawnAggression) spawnEnemy();

      if (Math.random() < (0.1 + wave * 0.012) * d.spawnMul * spawnAggression) spawnEnemy();

      if (Math.random() < (0.05 + wave * 0.006) * d.spawnMul * spawnAggression) spawnEnemy();

      if (isSnakeWave && Math.random() < 0.35) {
        spawnEnemy(undefined, undefined, Math.min(Math.floor(wave / 3) + 1, 8));
        if (Math.random() < 0.4) spawnEnemy(undefined, undefined, Math.min(Math.floor(wave / 3) + 1, 8));
      }
  ...
  for (let i = lasers.length - 1; i >= 0; i--) {
    ...
    // Player lasers can destroy enemy bullets (limited to 400 checks/frame for perf)

    const maxBulletChecks = Math.min(400, enemyBullets.length);

    const bulletStart = Math.max(0, enemyBullets.length - maxBulletChecks);

    for (let bi = enemyBullets.length - 1; bi >= bulletStart; bi--) {

      const eb = enemyBullets[bi];

      const bdx = l.x - eb.x, bdy = l.y - eb.y;

      const hitRadius = l.saber || l.beam ? 45 : 20;

      if (bdx * bdx + bdy * bdy < hitRadius * hitRadius) {
    ...
    if (l.y < topLimit) {
      ...
      l.rangeMul = 0.01 + 0.99 * Math.max(0, Math.min(1, pct));
  ...
  for (let i = enemies.length - 1; i >= 0; i--) {
    ...
    if (e.y < eDT - 200 || e.x < eDL - 300 || e.x > eDR + 300) { rmEnemy(i); continue; }
...
takeDamage('bullet');
    ...
    for (let j = 0; j < nearbyLasers.length; j++) {
      ...
      else if (l.ufo) hitRadius = Math.max(hitRadius, 22);
        ...
        if (l.bolt) {
            ...
            if (l.hitEnemies.indexOf(e) < 0) {
              ...
              if (e.hp <= 0 && !e.dead) { e.dead = true;
                ...
                if (dTier) { score += dTier.pt; playPopSnd(e.tier); spawnExplosion(e.x, e.y, dTier.cl[0], 8); spawnPowerup(e.x, e.y); if (Math.random() < ((getDifficulty().dropChance || 0.4) * 0.25)) spawnSpecialPowerup(e.x, e.y); updateUI(); }
      ...
      if (l.prism) {
            for (const pe of enemies) { 
                ...
                if (pd < 25) { 
                    ...
                    if (pe.hp <= 0 && !pe.dead) {
                        ...
                        if (pTier) { score += pTier.pt; playPopSnd(pe.tier); spawnExplosion(pe.x, pe.y, pTier.cl[0], 8); spawnPowerup(pe.x, pe.y); if (Math.random() < ((getDifficulty().dropChance || 0.4) * 0.25)) spawnSpecialPowerup(pe.x, pe.y); updateUI(); }
        ...
        if (l.ufo) {
          ...
          if (l.ufoExplode) {
            for (const oe of enemies) {
              if (oe.dead) continue;
              const od = Math.hypot(oe.x - l.x, oe.y - l.y);
              if (od < l.explodeRadius) {
                oe.hp -= l.explodeDmg * rm;
              }
            }
            spawnExplosion(l.x, l.y, 0xff0044, l.explodeRadius * 0.4);
            spawnExplosion(l.x + (Math.random()-0.5)*10, l.y + (Math.random()-0.5)*10, 0xff4488, l.explodeRadius * 0.25);
            spawnExplosion(l.x + (Math.random()-0.5)*10, l.y + (Math.random()-0.5)*10, 0xff88aa, l.explodeRadius * 0.15);
          } else {
            spawnExplosion(l.x, l.y, 0xff0044, 25);
            spawnExplosion(l.x + (Math.random()-0.5)*10, l.y + (Math.random()-0.5)*10, 0xff4488, 15);
            spawnExplosion(l.x + (Math.random()-0.5)*10, l.y + (Math.random()-0.5)*10, 0xff88aa, 10);
          }

          const ft = new PIXI.Text('BOOM!', { fontFamily:'sans-serif',fontSize:16,fontWeight:'900',fill:0xff0044,dropShadow:true,dropShadowColor:0xff0000,dropShadowBlur:6 });
          ft.anchor.set(0.5); ft.position.set(l.x, l.y - 10); ft.life = 0.8; ft.vy = -1.2; fxC.addChild(ft); fxT.push(ft);
        ...
        if (e.hp <= 0 && !e.dead) { e.dead = true;
          ...
          if (l.saber && l.saberLvl >= 4) { score += pts * combo * 2; updateUI(); }
  ...
  for (let i = enemyBullets.length - 1; i >= 0; i--) {
    ...
    for (let pi = powerups.length - 1; pi >= 0; pi--) {
      ...
      if (pu.spawnTime && performance.now() - pu.spawnTime < 500) continue;
  ...
  for (let i = powerups.length - 1; i >= 0; i--) {
    ...
    // Auto-magnetize powerups toward player
    ...
    // Player proximity auto-collect
    const cdx = cannon.x - pu.x, cdy = cannon.y - pu.y;
  ...
  if (gameState === 'playing' && pauseBtn.style.display !== 'block') pauseBtn.style.display = 'block';
...
function initCrawl() {
  ...
  const endCrawl = () => {
      ...
      setTimeout(() => {
          ...
          showContinueOption();
          const diffOverlay = document.getElementById('diffOverlay');
          if (diffOverlay) diffOverlay.classList.add('show');
